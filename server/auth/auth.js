...
const updateUser = async (req, res) => {
    ...
    const { name, email, role } = req.body; // Yangilanadigan ma'lumotlarni olish

    try {
        const updatedUser = await User.findByIdAndUpdate(id, { name, email, role }, { new: true }); // Foydalanuvchini yangilash
        if (!updatedUser) {
            return res.status(404).json({ message: 'Foydalanuvchi topilmadi.' });
        }
        res.status(200).json(updatedUser); // Yangilangan foydalanuvchini qaytarish
    } catch (error) {
...
const deleteUser = async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({ message: 'Foydalanuvchi muvaffaqiyatli o\'chirildi.' }); // O'chirilgan foydalanuvchini tasdiqlash
    } catch (error) {
...
module.exports = { registerController, loginController, getUsers, createUser, updateUser, deleteUser };
