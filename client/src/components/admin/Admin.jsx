...
const AdminCrud = () => {
  ...
  const fetchAdmins = async () => {
    const token  = localStorage.getItem('token') 
    try {
      const response = await axios.get('http://localhost:5173/api/admins',
        {
          headers: {
  ...
  const createAdmin = async () => {
    const token = localStorage.getItem('token'); 
    try {
        const response = await axios.post('http://localhost:5173/api/admin',
  ...
  const updateAdmin = async (id) => {
    const token  = localStorage.getItem('token') 
    try {
      const response = await axios.put(`http://localhost:5173/api/admin/${id}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const deleteAdmin = async (id) => {
    const token  = localStorage.getItem('token') 
    try {
      await axios.delete(`http://localhost:5173/api/admin/${id}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
