...
const SuperadminPanel = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      const token = localStorage.getItem('token');
      try {
        const response = await axios.get('https://sinftest.onrender.com/api/dashboard', {
          headers: {
  ...
  const createUser = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await axios.post('https://sinftest.onrender.com/api/dashboard',
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const deleteUser = async (id) => {
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`https://sinftest.onrender.com/api/users/${id}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
